package com.dosimple.task.config.elasticjob.dynamic.service;
...
import com.dosimple.task.config.elasticjob.dynamic.bean.Job;
import com.dosimple.task.config.elasticjob.dynamic.util.GsonHelper;
import com.dosimple.task.config.elasticjob.parser.JobConfParser;
...
public class JobService {
	...
	public void addJob(Job job) {
		...
		JobCoreConfiguration coreConfig = 
				...
				.failover(job.getFailover())
				.jobParameter(job.getJobParameter())
				.misfire(job.getMisfire())
		...
		if (jobType.equals("DATAFLOW")) {
			typeConfig = new DataflowJobConfiguration(coreConfig, job.getJobClass(), job.getStreamingProcess());
		}
		...
		jobConfig = LiteJobConfiguration.newBuilder(typeConfig)
				...
				.disabled(job.getDisabled())
				.monitorPort(job.getMonitorPort())
				.monitorExecution(job.getMonitorExecution())
	        ...
	        PathChildrenCacheListener childrenCacheListener = new PathChildrenCacheListener() {
	            public void childEvent(CuratorFramework client, PathChildrenCacheEvent event) throws Exception {
	                ...
	                case CHILD_ADDED:
	                	...
	                	Job job = GsonHelper.parseObject(config, Job.class);
	                	addJob(job);
	                    break;
	                default:
	                    break;
